...
class Job(object):
    ...
    def accounting_time(self):
        if self.start and self.end:
            raw_time = max((self.end - self.start).total_seconds() * self.nproc, self.cput, self.walltime)
        else:
            raw_time = max(self.cput, self.walltime)
        return raw_time
...
def main():
    ...
    valid_job = all_jobs.loc[(start_timestamp < all_jobs['Start']) & (all_jobs['End'] < end_timestamp)]
    ...
    for user in all_user:
        ...
        for i in daterange(tmp_start, tmp_end + timedelta(1)):
            if tmp_date is None:
                ...
                tmp_job = user_job.loc[(date_to_datetime(tmp_date) <= user_job['Start']) & (user_job['End'] < date_to_datetime(i))]
