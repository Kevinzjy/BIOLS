...
class Circos(object):
    ...
    def __init__(self, data, length='length', start_theta=90, figsize=(10, 10), gap=0.5):
        '''
        ...
        self.regions['theta_start'] = [numpy.deg2rad(start_theta+l/self.len_per_degree+gap*i) for i, l in enumerate(cumlen)]
        # polar axis
...
#         cb['color'] = [cyto_colors[c] for c in cb.color]
    ...
    def draw_link(self, rad, gids, starts, ends, color=None, alpha=1., lw=.2, bezier_radius=0):
        '''
        Draw links
        Parameters:
            rad: float
        ...
        patch = PathPatch(path, edgecolor=color, facecolor=color, lw=lw, alpha=alpha)
